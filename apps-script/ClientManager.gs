...
function getActiveClient() {
  ...
  for (let i = 1; i < data.length; i++) {
    if (data[i][0] === true) { // Active checkbox
      ...
      const missing = [];
      if (!client.clientName) missing.push('Client Name');
      if (!client.sellerId) missing.push('Seller ID');
      if (!client.marketplace) missing.push('Marketplace');
      if (!client.marketplaceId) missing.push('Marketplace ID');
      ...
      if (missing.length > 0) {
        throw new Error(
          `Active client "${client.clientName}" is missing required fields:\n\n` +
          missing.join(', ') +
          '\n\nPlease fill in all required fields in Client Settings sheet.'
        );
      }
